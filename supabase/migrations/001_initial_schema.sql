...
-- Unique constraint: one pending agreement per sender-receiver-event
...
CREATE TRIGGER update_team_event_participations_updated_at BEFORE UPDATE ON team_event_participations
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_pre_match_agreements_updated_at BEFORE UPDATE ON pre_match_agreements
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
